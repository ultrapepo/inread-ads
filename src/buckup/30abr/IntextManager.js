      ...
      class IntextWaterfall {
        ...
        async startAuction(trigger) {
          if (this.wa && this.wa.cI) {
              ...
              this.wa.cI["gexp-intext-is-fallback"] = (this._displayRenderState?.isFallback) ? "true" : "false";
          ...
          return new Promise((resolve, reject) => {
            ...
            this.player.on("adserror", (evt) => {
              ...
              if (errCode === 901 || errCode === 400 || errCode === 1009) {
                ...
                // Track who failed with 901
                const ad = evt?.getAd?.() || this.player?.ima?.getAdsManager?.()?.getCurrentAd();
                if (ad && this.node && this.node.wa && this.node.wa.cI) {
                    this.node.wa.cI.failed_campaignId = ad.getAdId();
                    this.node.wa.cI.failed_advertiser = ad.getAdvertiserName();
                    
                    const rawType = typeof ad.getLineItemType === "function" ? ad.getLineItemType() : null;
                    this.node.wa.cI.failed_type = rawType ? rawType.toLowerCase() : "unknown";
                    
                    logIntext(`[Intext:Video:IMA] 901 failure tracked: ${ad.getAdvertiserName()} (${ad.getAdId()}) type: ${this.node.wa.cI.failed_type}`);
                }
              }
            });
